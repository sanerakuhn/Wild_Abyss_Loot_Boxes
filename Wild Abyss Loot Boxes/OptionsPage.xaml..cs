...
namespace Wild_Abyss_Loot_Boxes
{
    public partial class OptionsPage : ContentPage
    {
        ...
        public static event Action<List<MagicItem>, bool> MagicItemsUpdated;
                ...
                FileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            ...
            if (result != null)
            {
                ...
                {
                    Preferences.Set("MagicItemsJson", fileContent);
                    MagicItemsUpdated?.Invoke(magicItems, false);
                }
                else if (MagicItemsAddRadioButton.IsChecked)
                {
                    var existingItems = Preferences.Get("MagicItemsJson", string.Empty);
                    var existingList = string.IsNullOrEmpty(existingItems)
                        ? new List<MagicItem>()
                        : JsonConvert.DeserializeObject<List<MagicItem>>(existingItems);

                    existingList.AddRange(magicItems);
                    Preferences.Set("MagicItemsJson", JsonConvert.SerializeObject(existingList));
                    MagicItemsUpdated?.Invoke(existingList, true);
                }

                await DisplayAlert("Success", "Magic items updated successfully!", "OK");
            }
        }
